...
send(To, Msg, Latency, InFun, OutFun) -> 
	...
	AnnotatedMsg = {Msg, erlang:monotonic_time(), erlang:convert_time_unit(Latency, millisecond, native), OutFun}, 
	...
	receive 
		{Msg, SendTime, ExpectedDelay, OutFun} ->
			...
			if ActualDelay < ExpectedDelay ->
				timer:sleep(erlang:convert_time_unit(ExpectedDelay - ActualDelay, native, millisecond));
			true ->
				ok
			end,
