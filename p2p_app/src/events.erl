...
-callback on_process_state(Pid :: pid(), OldState :: term(), NewState :: term(), Time :: term(), VClock :: term()) -> ok.
-callback on_link_state(From :: pid(), To :: pid(), OldState :: term(), NewState :: term(), Time :: term(), VClock :: term()) -> ok.
-callback on_message(From :: pid(), To :: pid(), Msg :: term(), FromState :: term(), ToState :: term(), FromTime :: term(), ToTime :: term(), FromVClock :: term(), ToVClock :: term()) -> ok.
...
init(Module) ->
	ets:new(events_info, [set, named_table, public, {read_concurrency, true}, {write_concurrency, true}]),
...
link_state(From, To, NewState) -> 
    ...
    events_callback_server ! {callback_link_state, From, To, OldState, NewState, NewTime, NewVClock}.
...
received_annotated_msg(Msg) ->					% Msg must be a tuple whose last element is an annotation
	...
	events_callback_server ! {callback_message, From, To, ActualMsg, FromState, ToState, FromTime, NewToTime, FromVClock, NewToVClock},
	ok.
...
loop(Module) ->
	receive
		...
		{callback_message, From, To, Msg, FromState, ToState, FromTime, ToTime, FromVClock, ToVClock} ->
			Module:on_message(From, To, Msg, FromState, ToState, FromTime, ToTime, FromVClock, ToVClock),
			loop(Module);
