-module(vectorclock).
-export([new_clock/0, new_aux/0, increment/2, max/2, below_or_equal/2, update_on_clock_change/4, update_on_send/4, compress/1, compress/3, decompress/1]).
...
update_on_clock_change({AuxU, AuxS}, OldClock, NewClock, Owner) ->  
	...
	AffectedKeys = maps:fold(fun (Key, NewValue, Acc) ->
	...
	NewAuxU = lists:foldl(fun (Key, Acc) ->
...
compress(Clock, {AuxU, AuxS}, Target) ->
	...
	maps:fold(fun (Key, ClockValue, Acc) ->
