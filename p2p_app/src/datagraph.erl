...
-export([
    ...
    load/1,
    save/2,
    add_node/2, add_node/3, add_nodes/2,
    add_directed_edge/3, add_directed_edge/4, add_directed_edges/2,
    add_edge/3, add_edge/4, add_edges/2,
    get_node_data/2, get_edge_data/3,
    get_list_of_nodes/1, get_list_of_datanodes/1,
    get_list_of_adjs/2, get_list_of_dataadjs/2,
    get_list_of_edges/1, get_list_of_dataedges/1
]).
...
load(Filename) ->
    ...
    G0 = new(),
    G1 = lists:foldl(fun (Elem, Acc) ->
                         case Acc of
                             {badformat, Elem} -> Acc;
                             _ -> case jsondata_to_node(Elem) of
                     ...
                     G0, maps:get(<<"nodes">>, JsonData)),
    G2 = lists:foldl(fun (Elem, Acc) ->
                         case Acc of
                             {badformat, Elem} -> Acc;
                             _ -> case jsondata_to_edge(Elem) of
                                      undefined -> {badformat, Elem};
...
get_list_of_edges(G) ->
    maps:fold(fun (V1, {_Data, Adjs}, Acc) -> 
...
get_list_of_dataedges(G) ->
    maps:fold(fun (V1, {_Data, Adjs}, Acc1) -> 
...
jsondata_to_node(JsonData) ->
    case JsonData of
        ...
        [V] -> {V, undefined};
        [V, Data] -> {V, Data};
        [V | Data] -> {V, list_to_tuple(Data)};
        M when is_map(M) ->
            case {maps:get(<<"id">>, M, undefined), 
                  maps:get(<<"data">>, M, undefined)} of
                {undefined, _} -> undefined;
                {V, undefined} -> {V, undefined};
                {V, Data} -> {V, Data}
            end;
        {PropList} -> 
            case {proplists:get_value(<<"id">>, PropList), 
                  proplists:get_value(<<"data">>, PropList)} of 
                {undefined, _} -> undefined;
                {V, undefined} -> {V, undefined};
                {V, Data} -> {V, Data}
            end;
...
jsondata_to_edge(JsonData) ->
    case JsonData of
        [V1, V2] -> {V1, V2, undefined};
        [V1, V2, Data] -> {V1, V2, Data};
        [V1 | [V2 | Data]] -> {V1, V2, list_to_tuple(Data)};
        M when is_map(M) ->
            case {maps:get(<<"src">>, M, undefined), 
        ...
        {PropList} ->
            case {proplists:get_value(<<"src">>, PropList), 
