...
def generate(num_nodes):
    ...
    for i in range(len(positions)):
        ...
        while any(distance(positions[i], q) < 0.2 for q in positions[:i]):
            ...
            c += 1
            if c > 10000:
                raise "Cannot find space for new point"
    ...
    for simplex in vor.ridge_vertices:
        if -1 not in simplex:
            ...
            if 0 <= p[0] <= 1 and 0 <= p[1] <= 1 and 0 <= q[0] <= 1 and 0 <= q[1] <= 1:
                dist = distance(p, q)
                graph.add_node(tuple(p), pos=p)
                graph.add_node(tuple(q), pos=q)
                graph.add_edge(tuple(p), tuple(q), weight=dist)
    return graph
...
def find_nearest_node(graph, pos, x, y):
    distances = [(node, (x - pos[node][0])**2 + (y - pos[node][1])**2) for node in graph.nodes()]
...
if __name__ == "__main__":
    ...
    if not args.quiet:
        ...
        fig.canvas.mpl_connect('motion_notify_event', lambda event: on_motion(event, graph, pos))
        plt.show()
